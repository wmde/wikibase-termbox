...
describe( 'Fingerprint.vue', () => {
	...
	it( 'renders the component with primarayLanguage as modifier if isModified flag is true', () => {
	...
	it( 'renders the component with no modifier if isModified flag is false', () => {
		const wrapper = shallowMount(
			Fingerprint,
			{
				store,
		...
		expect( wrapper.classes().length === 1 ).toBeTruthy();
	} );

	it( 'renders the component with no modifier if isModified flag is not set', () => {
		const wrapper = shallowMount(
			Fingerprint,
			{
				store,
