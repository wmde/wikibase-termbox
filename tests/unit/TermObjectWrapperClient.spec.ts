...
const Wrapper = new TermObjectWrapperClient( JSON.stringify( MockData.default ) );
const EmptyWrapper = new TermObjectWrapperClient( '{"id":"Q123", "type":"item" }' );

describe( 'TermObjectWrapperClient.ts', () => {
	it( 'throws an error when the term has no id', () => {
		expect( () => {
			const dummy = new TermObjectWrapperClient( '' );
		...
		expect( () => {
			const dummy = new TermObjectWrapperClient( 'justATest' );
		} ).toThrowError( SyntaxError );
	} );

	it( 'throws an error when the term has no type', () => {
		expect( () => {
			const dummy = new TermObjectWrapperClient( '{"id": "Q123"}' );
		} ).toThrowError( InvalidTermException );
	} );

	it( 'has a id', () => {
		expect( Wrapper.getId() ).toMatch( 'Q64' );
	} );

	it( 'has a term type', () => {
		expect( Wrapper.getType() ).toMatch( 'item' );
	} );

	it( 'has labels', () => {
		expect( Wrapper.getLabels() ).toStrictEqual(
	...
	it( 'has label by languageKey', () => {
		expect( Wrapper.getLabelByLanguageKey( 'de' ) ).toStrictEqual( 'Berlin' );
	} );
	...
	it( 'has descriptions', () => {
		expect( Wrapper.getDescriptions() ).toStrictEqual(
			JSON.parse( /* tslint:disable-next-line:max-line-length */
	...
	it( 'has aliases', () => {
		expect( Wrapper.getAliases() ).toStrictEqual(
			JSON.parse( /* tslint:disable-next-line:max-line-length */
